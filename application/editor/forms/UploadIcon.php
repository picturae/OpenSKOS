...
class Editor_Forms_UploadIcon extends Zend_Form
{
    ...
    protected function buildFileUpload()
    {
        ...
        $tenantCode = Zend_Auth::getInstance()->getIdentity()->tenant;
        ...
        if (null !== $tenantCode) {
            ...
            if (isset($editorOptions['schemeIcons']) && isset($editorOptions['schemeIcons']['uploadPath'])) {
                $iconUpload->setDestination(APPLICATION_PATH . $editorOptions['schemeIcons']['uploadPath'] . '/' . $tenantCode);
            } else {
                $iconUpload->setDestination(APPLICATION_PATH . self::DEFAULT_UPLOAD_PATH . '/' . $tenantCode);
            }
